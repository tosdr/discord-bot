...
module.exports = {
	...
	async execute(interaction, confirmed = false) {
		...
		if (!confirmed) {
			...
			const userID = parseInt(interaction.customId.replace('phoenixPurgeConfirm-', ''));
			const options = {
				method: 'DELETE',
				headers: {apiKey: tosdrApiKey}
			};
